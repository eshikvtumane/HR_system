...
def phone_format(value):
    ...
    if phone_len == 4:
        return '(гор) -  %s‒%s' % (value[:2],value[2:])
    if phone_len == 5:
        return '(гор) -  %s‒%s‒%s' % (value[:1],value[1:3], value[3:])
    if phone_len == 6:
        return '(гор) -  %s‒%s‒%s' % (value[:2], value[2:4], value[4:])
    if phone_len == 7:
        return '(гор) -  %s‒%s‒%s' % (value[:3], value[3:5], value[5:])
    if phone_len == 10:
